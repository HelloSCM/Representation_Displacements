...
def freeze_unused_tail_for_block_feature_training(model: torch.nn.Module) -> List[str]:
    """Freeze ViT parameters that are downstream of block outputs.

    Distillation losses use the raw block output, e.g. the last Transformer block
    residual stream, rather than the final normalized/classifier output. In DDP,
    parameters such as ``norm.weight`` and ``norm.bias`` are therefore unused by
    the loss and can trigger:
        Expected to have finished reduction in the prior iteration...

    Freezing these downstream parameters keeps the experiment faithful to the
    requested representation loss and avoids unnecessary DDP unused-parameter
    handling. Returns the names that were frozen for logging.
    """
    base = model.base_model if isinstance(model, ViTBlockFeatureExtractor) else model
    frozen = []
    downstream_prefixes = (
    ...
    for name, param in base.named_parameters():
        ...
        if first in downstream_prefixes:
